...
model = load_object_model(
    ...
    features=("rgba","pose_vectors_flat","principal_curvatures_log"),            # 색상까지 함께 읽기
...
colors = plt.cm.plasma(1-brightness)  # inferno / viridis / plasma / 등
...
# magnitude
kappa_max = model.principal_curvatures_log
...
# print(len(kappa_max))
colors = plt.cm.inferno(1-kappa_max[:,0])  # inferno / viridis / plasma / 등
fig3 = plt.figure(figsize=(6, 6))
cx  = fig.add_subplot(111, projection="3d")
cx.scatter(model.x, model.y, model.z,
           c=colors, s=8, linewidth=0)

cx.set_xlabel("x"); ax.set_ylabel("y"); ax.set_zlabel("z")
cx.set_box_aspect([1, 0.1, 1])       # 큐브 비율
